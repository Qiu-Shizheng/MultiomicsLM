...
class OmicsDiseaseDataset(Dataset):
    def __init__(self, data: Dict[str, Any], labels: pd.DataFrame, config, id_col: str = "sample_id"):
        self.data = data
        self.labels = labels.reset_index(drop=True)
        self.config = config
        self.id_col = id_col
        self.index = {str(sample_id): i for i, sample_id in enumerate(data["sample_ids"].astype(str).tolist())}
        self.indices = np.asarray([self.index[str(sample_id)] for sample_id in self.labels[id_col].astype(str).tolist()], dtype=np.int64)
    ...
    def __getitem__(self, idx):
        ...
        return {
            "sample_id": str(self.labels.iloc[idx][self.id_col]),
...
def parse_labels(labels_path: str, id_col: str = "sample_id", label_col: Optional[str] = None, baseline_date_col: Optional[str] = None, diagnosis_date_col: Optional[str] = None, task: str = "baseline") -> pd.DataFrame:
    df = read_table(labels_path, id_col=id_col)
    ...
    if label_col is not None:
        ...
        return out[[id_col, "label"]].reset_index(drop=True)
    ...
    out = df.loc[include, [id_col]].copy()
...
def run_epoch(model, loader, criterion, optimizer, device, train: bool, use_amp: bool, grad_clip: float):
    ...
    return avg_loss, np.asarray(all_sample_ids), y_true, y_prob
...
def predict(model, loader, device, use_amp: bool):
    ...
    return np.asarray(all_sample_ids), np.concatenate(all_y), np.concatenate(all_prob), np.concatenate(all_feat)
...
def finetune_disease_model(
    ...
    data = client.load_arrays(protein_path=protein_path, metabolite_path=metabolite_path, id_col=id_col)
    labels = parse_labels(
    ...
    omics_sample_ids = set(data["sample_ids"].astype(str).tolist())
    labels = labels[labels[id_col].astype(str).isin(omics_sample_ids)].copy()
    labels = labels.drop_duplicates(subset=[id_col]).reset_index(drop=True)
    if len(labels) < 20:
    ...
    train_ds = OmicsDiseaseDataset(data, train_df, client.config, id_col=id_col)
    val_ds = OmicsDiseaseDataset(data, val_df, client.config, id_col=id_col)
    ...
    for epoch in range(max_epochs):
        train_loss, train_sample_ids, train_y, train_prob = run_epoch(model, train_loader, criterion, optimizer, client.device, True, use_amp, grad_clip)
        val_loss, val_sample_ids, val_y, val_prob = run_epoch(model, val_loader, criterion, optimizer, client.device, False, use_amp, grad_clip)
    ...
    train_sample_ids, train_y, train_prob, train_feat = predict(model, train_loader_eval, client.device, use_amp)
    val_sample_ids, val_y, val_prob, val_feat = predict(model, val_loader_eval, client.device, use_amp)

    train_metrics = compute_metrics(train_y, train_prob, threshold)
    val_metrics = compute_metrics(val_y, val_prob, threshold)

    pred_train = pd.DataFrame({
        id_col: train_sample_ids,
        "split": "train",

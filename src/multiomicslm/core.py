...
def read_table(path: str, id_col: str = "sample_id") -> pd.DataFrame:
    ...
    for c in df.columns:
        name = str(c)
        is_unnamed = name.startswith("Unnamed:") or name.strip() == ""
        if is_unnamed and df[c].isna().all():
            drop_cols.append(c)
    if drop_cols:
        df = df.drop(columns=drop_cols)
    if id_col not in df.columns:
        df.insert(0, id_col, np.arange(len(df)).astype(str))
...
def numeric_matrix(df: pd.DataFrame, id_col: str) -> Tuple[np.ndarray, List[str]]:
...
class MultiomicsLM:
    ...
    def load_arrays(self, protein_path: Optional[str] = None, metabolite_path: Optional[str] = None, id_col: str = "sample_id") -> Dict[str, Any]:
        protein_df = read_table(protein_path, id_col) if protein_path else None
        metabolite_df = read_table(metabolite_path, id_col) if metabolite_path else None

        if protein_df is not None and metabolite_df is not None:
            protein_ids = protein_df[id_col].astype(str).tolist()
            metabolite_ids = set(metabolite_df[id_col].astype(str).tolist())
            common = [x for x in protein_ids if x in metabolite_ids]
            if len(common) == 0:
                raise ValueError("No overlapping sample IDs between protein and metabolite files")
            protein_df = protein_df.set_index(id_col).loc[common].reset_index()
            metabolite_df = metabolite_df.set_index(id_col).loc[common].reset_index()
            sample_ids = common
        elif protein_df is not None:
            sample_ids = protein_df[id_col].astype(str).tolist()
        elif metabolite_df is not None:
            sample_ids = metabolite_df[id_col].astype(str).tolist()
        else:
            raise ValueError("At least one of protein_path or metabolite_path must be provided")

        out = {"sample_ids": np.asarray(sample_ids, dtype=str)}

        if protein_df is not None:
            raw_p, p_cols = numeric_matrix(protein_df, id_col)
        ...
        if metabolite_df is not None:
            raw_m, m_cols = numeric_matrix(metabolite_df, id_col)
    ...
    def impute_file(self, protein_path: Optional[str] = None, metabolite_path: Optional[str] = None, output_path: str = "imputed.csv", mode: str = "auto", id_col: str = "sample_id", batch_size: int = 128, n_iters: int = 3) -> pd.DataFrame:
        data = self.load_arrays(protein_path, metabolite_path, id_col=id_col)
    ...
    def embed_file(self, protein_path: Optional[str] = None, metabolite_path: Optional[str] = None, output_path: str = "embeddings.csv", mode: str = "fused", id_col: str = "sample_id", batch_size: int = 128) -> pd.DataFrame:
        data = self.load_arrays(protein_path, metabolite_path, id_col=id_col)
        modes = ["protein", "metabolite", "fused"] if mode == "all" else [mode]
        result = pd.DataFrame({id_col: data["sample_ids"]})
        for m in modes:
